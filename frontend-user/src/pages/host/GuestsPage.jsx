...
const GuestsPage = () => {
  ...
  const [newGuest, setNewGuest] = useState({ name: "", phone: "", group: "គ្រួសារ", status: "រង់ចាំ" });
  ...
  const counts = {
    total: guests.length,
    confirmed: guests.filter(g => g.status === "បញ្ជាក់").length,
    pending: guests.filter(g => g.status === "រង់ចាំ").length,
    rejected: guests.filter(g => g.status === "បដិសេធ").length,
  };
  ...
  const handleAddGuest = (e) => {
    ...
    setNewGuest({ name: "", phone: "", group: "គ្រួសារ", status: "រង់ចាំ" });
  ...
  return (
    <div className="gp-page">
      ...
      <div className="gp-header">
        ...
        <button className="gp-add-btn" onClick={() => setShowAddModal(true)}>+ បន្ថែមភ្ញៀវ</button>
      </div>
      ...
      {/* Add Guest Modal */}
      {showAddModal && (
        <div className="gp-modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="gp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="gp-modal-header">
              <h2>បន្ថែមភ្ញៀវថ្មី</h2>
              <button className="gp-modal-close" onClick={() => setShowAddModal(false)}>✕</button>
            </div>
            <form onSubmit={handleAddGuest} className="gp-modal-form">
              ...
              <div className="gp-modal-field">
                ...
                >
                  <option>គ្រួសារ</option>
                  <option>មិត្តភក្ដិ</option>
              ...
              <div className="gp-modal-field">
                ...
                >
                  <option>រង់ចាំ</option>
                  <option>បញ្ជាក់</option>
                  <option>បដិសេធ</option>
                </select>
              </div>
              <div className="gp-modal-actions">
                <button type="button" className="gp-modal-cancel" onClick={() => setShowAddModal(false)}>បោះបង់</button>
                <button type="submit" className="gp-modal-submit">បន្ថែម</button>
              </div>
            </form>
          </div>
        </div>
