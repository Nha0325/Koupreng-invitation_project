...
/**
 * ForgotPasswordPage — 4-step wizard
 *
 *   1. StepEmail        → identifier (email or phone) → request OTP
 *   2. StepOTP          → verify the 6-digit code → mint a reset token
 *   3. StepNewPassword  → submit a new password against the reset token
 *   4. StepSuccess      → confirmation + link back to /login
 *
 * In UI-only mode the OTP is generated by `inMemoryDb` and printed to the
 * console (and surfaced as a small dev hint on the OTP step), so the flow can
 * be exercised end-to-end without a mail/SMS provider.
 */
...
const StepEmail = ({ onNext }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!value) {
      setError("សូមបំពេញប្រអប់");
      return;
    }
    setError("");
    setSubmitting(true);
    try {
      const { devOtp } = await authService.forgotPassword({ email: value });
      onNext(value, devOtp);
    } catch (err) {
      setError(err?.message || "មិនអាចផ្ញើ OTP បានទេ");
    } finally {
      setSubmitting(false);
    }
  };
...
/* ── Step 2 — OTP ── */
const StepOTP = ({ identifier, devOtp, onNext, onResend }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const { resetToken } = await authService.verifyOtp({ identifier, code });
      onNext(resetToken);
    } catch (err) {
      setError(err?.message || "OTP មិនត្រឹមត្រូវ");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="auth-form">
      <div className="auth-field">
        ...
        {/* Dev hint — UI-only mode prints the code instead of sending email/SMS. */}
        {devOtp && (
      ...
      <button type="submit" disabled={!filled || submitting} className="auth-submit">
        {submitting ? "កំពុងបញ្ជាក់..." : "បញ្ជាក់ OTP"}
      </button>
    </form>
  );
};
...
/* ── Step 3 — New Password ── */
const StepNewPassword = ({ resetToken, onDone }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.message || "មិនអាចកំណត់លេខសម្ងាត់ឡើងវិញបានទេ");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="auth-form">
      ...
      <button
        type="submit"
        disabled={!!mismatch || !password || !confirm || submitting}
        className="auth-submit"
      >
        {submitting ? "កំពុងរក្សាទុក..." : "កំណត់លេខសម្ងាត់ឡើងវិញ"}
      </button>
    </form>
  );
};
...
const ForgotPassword = () => {
  ...
  const handleResend = async () => {
    ...
    try {
      const { devOtp: code } = await authService.forgotPassword({ email: identifier });
      setDevOtp(code);
    } catch {
      // Resend is best-effort; the user can also go back a step.
    }
  };
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        {step === 3 && (
          <StepNewPassword resetToken={resetToken} onDone={() => setStep(4)} />
        )}
        {step === 4 && <StepSuccess />}
