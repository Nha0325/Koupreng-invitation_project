...
import AddGuestModal from "../../shared/components/modals/AddGuestModal";
...
const GuestsPage = () => {
  ...
  const handleAddGuest = (newGuest) => {
    const guestWithId = {
      ...newGuest,
      id: guests.length + 1,
      amount: newGuest.amount ? `$${newGuest.amount}` : "—",
      seat: newGuest.seat || "—",
    };
          ...
          <tbody>
            {filtered.map((g) => (
              <tr key={g.id}>
                ...
                <td>{g.group}</td>
                <td>
                  <span className={`gp-status ${statusColor[g.status]}`}>
                    {g.status}
                  </span>
                </td>
                <td className={g.amount === "—" ? "gp-muted" : "gp-amount"}>
                  {g.amount}
                </td>
