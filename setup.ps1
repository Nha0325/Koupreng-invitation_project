...
function Get-ManualInstallHint {
    ...
    switch ($Tool) {
        ...
        "Apache Maven" { return "Install Maven from https://maven.apache.org/download.cgi or rely on backend\\mvnw.cmd for backend commands." }
...
try {
    ...
    if (Test-Path -LiteralPath $backendEnv) {
        ...
        elseif ($dbUrl -notmatch '^jdbc:mysql://') {
            Add-Issue "backend\.env" "DB_URL is not a MySQL JDBC URL. This project now uses MySQL only." "Edit backend\.env so DB_URL starts with jdbc:mysql://, then rerun setup.ps1."
        }
    ...
    foreach ($legacyFrontendEnv in @(
        (Join-Path $ProjectRoot "frontend-user\.env"),
        (Join-Path $ProjectRoot "frontend-admin\.env")
    )) {
...
try {
    ...
    elseif (Test-Path -LiteralPath (Join-Path $ProjectRoot "backend\mvnw.cmd")) {
        Set-Summary "Maven" "Global mvn missing; backend Maven wrapper is available"
        Write-WarnLine "Global mvn is missing, but backend\mvnw.cmd is available."
    }
    else {
        Set-Summary "Maven" "Missing"
        Add-Issue "Apache Maven" "Neither mvn nor backend\mvnw.cmd was found." (Get-ManualInstallHint "Apache Maven")
    }
...
elseif ($SkipDatabaseSetup) {
    ...
    try {
        ...
        elseif (-not (Test-Path -LiteralPath $backendEnv)) {
            ...
            Add-Issue "Database" "backend\.env does not exist." "Copy backend\.env.example to backend\.env, update DB credentials, then rerun setup.ps1."
        }
        else {
            $dbUrl = Get-EnvValue $backendEnv "DB_URL"
            ...
            Invoke-MySqlSql `
                ...
                -Sql "SELECT 1;" `
                -FailureMessage "Could not connect to MySQL using backend\.env credentials."
        ...
        Add-Issue "Database" $_.Exception.Message "Update backend\.env with the correct DB_USERNAME and DB_PASSWORD, make sure MySQL is running, then rerun setup.ps1."
    }
}
...
if ($RunBackend) {
    Start-DevCommand -Label "backend" -WorkingDirectory (Join-Path $ProjectRoot "backend") -Command ".\mvnw.cmd spring-boot:run"
}

if ($RunFrontendUser) {
    Start-DevCommand -Label "frontend user" -WorkingDirectory (Join-Path $ProjectRoot "frontend-user") -Command "npm run dev"
}

if ($RunFrontendAdmin) {
    Start-DevCommand -Label "frontend admin" -WorkingDirectory (Join-Path $ProjectRoot "frontend-admin") -Command "npm run dev"
}
