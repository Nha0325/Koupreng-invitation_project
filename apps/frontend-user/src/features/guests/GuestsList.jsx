...
function normalizeBackendRsvp(entry) {
  return {
    id: entry.id,
  ...
  useEffect(() => {
    ...
    async function loadGuests() {
      ...
      try {
        ...
        const [backendGuests, backendRsvps] = await Promise.all([
          guestService.listByInvitation(selectedInvitationId),
          rsvpService.listByInvitation(selectedInvitationId),
        ]);
        if (!active) return;
        setManualGuests((backendGuests || []).map(normalizeBackendGuest));
        setRsvpGuests((backendRsvps || []).map(normalizeBackendRsvp));
      } catch (err) {
        if (active) {
