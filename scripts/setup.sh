...
Options:
  ...
  --run-frontend-user     Start apps/frontend-user after setup.
  --run-frontend-admin    Start apps/frontend-admin after setup.
  --help                  Show this help.
EOF
}
...
if [ "$SKIP_FRONTEND" -eq 1 ]; then
    ...
    if install_frontend_deps "$PROJECT_ROOT/apps/frontend-user" "apps/frontend-user"; then
        SUMMARY_FRONTEND_USER_DEPS="Ready"
    else
        SUMMARY_FRONTEND_USER_DEPS="Failed"
    fi

    if install_frontend_deps "$PROJECT_ROOT/apps/frontend-admin" "apps/frontend-admin"; then
...
elif [ ! -f "$PROJECT_ROOT/apps/backend/mvnw" ]; then
    SUMMARY_BACKEND="Missing apps/backend/mvnw"
    add_issue "Backend check" "apps/backend/mvnw was not found." "Restore apps/backend/mvnw, then rerun ./setup.sh."
else
    if [ ! -x "$PROJECT_ROOT/apps/backend/mvnw" ]; then
        chmod +x "$PROJECT_ROOT/apps/backend/mvnw"
        info "Made apps/backend/mvnw executable"
    fi
    SUMMARY_BACKEND="Ready"
    info "apps/backend/mvnw is ready"
fi
...
if [ "${#ISSUES[@]}" -eq 0 ]; then
    if [ "$RUN_BACKEND" -eq 1 ]; then
        start_background "backend" "$PROJECT_ROOT/apps/backend" ./mvnw spring-boot:run
    fi
    if [ "$RUN_FRONTEND_USER" -eq 1 ]; then
        start_background "apps/frontend-user" "$PROJECT_ROOT/apps/frontend-user" npm run dev
    fi
    if [ "$RUN_FRONTEND_ADMIN" -eq 1 ]; then
        start_background "apps/frontend-admin" "$PROJECT_ROOT/apps/frontend-admin" npm run dev
    fi
