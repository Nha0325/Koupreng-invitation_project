...
ROOT_CORS_ALLOWED_ORIGINS="$(env_file_value "$PROJECT_ROOT/.env" CORS_ALLOWED_ORIGINS)"
PUBLIC_FRONTEND_URL="$(env_file_value "$PROJECT_ROOT/frontend-user/.env" VITE_PUBLIC_APP_URL)"
...
if [ "$USE_BOT" = true ] && [ "$TELEGRAM_BOT_PORT" = "$BACKEND_PORT" ]; then
  ...
  echo "[DEV] Run with TELEGRAM_BOT_PORT=8000 ./dev.sh or choose another free bot port." >&2
  exit 1
fi
